  ...
  function(record, error, search, format, email) {
    ...
    function validateLine(context) {
      ...
      if (salesOrderLine) {
        ...
        if (itemOptions.length != 0) {
          ...
          var variables_list = ['Cloud amount (at 08:00, 14:00 & 20:00)',
            'Cloud types (at 08:00, 14:00 & 20:00)', 'Cloudiness',
            'Dew-point temperature',
            'Dew-point temperature (at 08:00, 14:00 & 20:00)',
            'Dry-bulb temperature',
            'Dry-bulb temperature (at 08:00, 14:00 & 20:00)',
            'Height of pressure level', 'Maximum temperature',
            'Minimum temperature', 'Pressure', 'Rain', 'Rainfall',
            'Relative humidity',
            'Relative humidity (at 08:00, 14:00 & 20:00)',
            'Relative humidity at 1.5 m', 'Sunshine', 'Surface pressure',
            'Temperature', 'Temperature at 1.5 m', 'Wet-bulb temperature',
            'Wet-bulb temperature (at 08:00, 14:00 & 20:00)',
            'Wind direction', 'Wind direction (at 08:00, 14:00 & 20:00)',
            'Wind direction at 10 m', 'Wind gust', 'Wind speed',
            'Wind speed (at 08:00, 14:00 & 20:00)', 'Wind speed (scalar)',
            'Wind speed (vector)', 'Wind speed at 10 m',
            'Pressure (at 08:00, 14:00 & 20:00)', 'Minimum humidity',
            'Maximum humidity', 'Minimum pressure', 'Maximum pressure'
          ];
          var optionList = set_of_option_array.map(function(result) {
            return {
              key: result.split(ext_String)[2],
              kvalue: result.split(ext_String)[3],
              is_variable: variables_list.indexOf(result.split(ext_String)[
                2])
            };
          });
          ...
          staticItems.forEach(function(staticItems) {
            new_description = new_description + staticItems.key + ': ' +
              staticItems.kvalue + String.fromCharCode(13);
          ...
          if (itemData.parentItem == 'Forecast') {
            ...
            if (itemData.periodType != 'Years' && itemData.periodType !=
              'Months' && itemData.periodType != 'Days' &&
              itemData.periodType != 'Images' && itemData.periodType !=
              'Grid Points' && itemData.periodType != 'Radius') {
              calcData.quantity = inputData.size_period_type;
            } else {
              if (itemData.periodType == 'Radius' && calcData.period < 21) {
                calcData.quantity = 20;
              } else {
                if (itemData.periodType == 'Grid Points' && inputData.size_period_type <
                  9) {
                  calcData.quantity = 8;
                } else {
                  if (itemData.periodType == 'Images' || itemData.periodType ==
                    'Days') {
          ...
          if (itemData.parentItem == 'Forecast') {
            calcData.amount = itemData.basePrice * calcData.variables *
              calcData.quantity * calcData.period * calcData.hours * calcData
              .days;
          } else {
            if (itemData.periodType == 'Grid Points') {
              calcData.amount = calcData.base_maps + (inputData.points *
                itemData.basePrice * calcData.variables * calcData.quantity
              ) * calcData.levels;
            } else {
              calcData.amount = inputData.points * itemData.basePrice *
                calcData.variables * calcData.quantity * calcData.levels;
            }
          }
          new_description = new_description + 'Options:';
          selectedVariables.forEach(function(selectedVariables) {
            new_description = new_description + String.fromCharCode(13) +
              selectedVariables.key
          });
          if (m) {
            new_description = new_description + String.fromCharCode(13) + m +
              ' months';
          }
    ...
    function saveRecord(context) {
      ...
      var customerRecord = record.load({
        type: 'customer',
        id: quoteRecord.getValue('entity')
      });
      ...
      quoteRecord.setValue('custbody41', accountNumber)
    }
    return {
